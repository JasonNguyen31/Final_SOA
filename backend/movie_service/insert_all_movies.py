"""
Script to insert all movies to the database
"""
import asyncio
...
async def main():
    ...
    print("🎬 Inserting movies to database...")
    print(f"Database: {DATABASE_NAME}")

    inserted_count = 0
    for movie in movies_data:
        # Delete if exists
        await movies_collection.delete_one({"title": movie["title"]})

        # Insert
        result = await movies_collection.insert_one(movie)
        print(f"✅ Inserted: {movie['title']} ({movie['releaseYear']}) - Country: {movie['country']}")
        inserted_count += 1

    print(f"\n🎉 Done! Inserted {inserted_count} movies!")
